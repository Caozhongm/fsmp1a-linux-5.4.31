...
#define MAX30102_REG_FIFO_DATA_BYTES		6  //原为3
...
#define MAX30102_REG_SPO2_CONFIG_SR_400HZ	0x00 //0x03
...
struct max30102_data {
	...
	u8 buffer[12];
	unsigned int processed_buffer[3]; 
	//__be16 processed_buffer[3]; /* 3 x 18-bit (padded to 32-bits) */
};
...
{
	int ret;
	unsigned short un_temp = 0;
	unsigned short fifo_red = 0;
	unsigned short fifo_ir = 0;
...
//	printk("driver | fifo_red = %x * fifo_ir = %x\n",fifo_red,fifo_ir);

	data->processed_buffer[0] = fifo_red;
	data->processed_buffer[1] = fifo_ir;
#if 0
	switch (measurements) {
	case 3:
...
#endif	
	ret = regmap_write(data->regmap, MAX30102_REG_FIFO_CONFIG,0x0f);
...
#endif
	return regmap_write(data->regmap, MAX30102_REG_INT_ENABLE,0xc0);
}
